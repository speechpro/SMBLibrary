...
namespace SMBLibrary
{
    ...
    public class FileFsObjectIdInformation : FileSystemInformation
    {
        ...
        public FileFsObjectIdInformation(Span<byte> buffer, int offset)
        {
            LittleEndianConverter.ToGuid(buffer, offset + 0);
            ExtendedInfo = ByteReader.ReadBytes_RentArray(buffer, offset + 16, 48);
        }

        public override void WriteBytes(Span<byte> buffer, int offset)
        {
            LittleEndianWriter.WriteGuidBytes(buffer, offset + 0, ObjectID);
            BufferWriter.WriteBytes(buffer, offset + 16, ExtendedInfo);
        }
