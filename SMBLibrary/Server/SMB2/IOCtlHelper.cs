...
namespace SMBLibrary.Server.SMB2
{
    internal class IOCtlHelper
    {
        internal static SMB2Command GetIOCtlResponse(IOCtlRequest request, ISMBShare share, SMB2ConnectionState state)
        {
            ...
            var ctlCode = Enum.IsDefined(typeof(IoControlCode), request.CtlCode) ? ((IoControlCode)request.CtlCode).ToString() : ("0x" + request.CtlCode.ToString("X8"));
            if (!request.IsFSCtl)
            {
                ...
                request.CtlCode == (uint)IoControlCode.FSCTL_DFS_GET_REFERRALS_EX)
            {
                ...
                return ObjectsPool<ErrorResponse>.Get().Init(request.CommandName, NTStatus.STATUS_FS_DRIVER_REQUIRED);
            }
            ...
            var maxOutputLength = (int)request.MaxOutputResponse;
            IMemoryOwner<byte> output;
            var status = share.FileStore.DeviceIOControl(handle, request.CtlCode, request.Input, out output, maxOutputLength);
            ...
            var response = ObjectsPool<IOCtlResponse>.Get().Init();
