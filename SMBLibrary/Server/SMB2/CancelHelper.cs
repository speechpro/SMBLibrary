...
namespace SMBLibrary.Server.SMB2
{
    internal class CancelHelper
    {
        internal static SMB2Command GetCancelResponse(CancelRequest request, SMB2ConnectionState state)
        {
            ...
            {
                var context = state.GetAsyncContext(request.Header.AsyncId);
                if (context != null)
                {
                    var share = session.GetConnectedTree(context.TreeID);
                        ...
                        status == NTStatus.STATUS_NOT_SUPPORTED) // See ChangeNotifyHelper.cs
                    {
                        ...
                        var response = ObjectsPool<ErrorResponse>.Get().Init(request.CommandName, NTStatus.STATUS_CANCELLED);
