...
namespace SMBLibrary.Server.SMB1
{
    internal class NTCreateHelper
    {
        internal static SMB1Command GetNTCreateResponse(SMB1Header header, NTCreateAndXRequest request, ISMBShare share, SMB1ConnectionState state)
        {
            ...
            var isExtended = (request.Flags & NTCreateFlags.NT_CREATE_REQUEST_EXTENDED_RESPONSE) > 0;
            ...
            var fileAttributes = ToFileAttributes(request.ExtFileAttributes);
            ...
            var createStatus = share.FileStore.CreateFile(out handle, out fileStatus, Arrays.RentFrom<char>(path), desiredAccess, fileAttributes, request.ShareAccess, request.CreateDisposition, request.CreateOptions, session.SecurityContext);
            ...
            if (isExtended)
            {
                var response = CreateResponseExtendedFromFileInformation(fileInfo, fileID.Value, fileStatus);
                return response;
            }
            else
            {
                var response = CreateResponseFromFileInformation(fileInfo, fileID.Value, fileStatus);
                return response;
            }
        }
        ...
        {
            var response = new NTCreateAndXResponse();
            response.FID = fileID;
        ...
        {
            var response = new NTCreateAndXResponseExtended();
            response.FID = fileID;
            response.CreateDisposition = ToCreateDisposition(fileStatus);
            response.ExtFileAttributes = ExtendedFileAttributes.Normal;
            response.ResourceType = ResourceType.FileTypeMessageModePipe;
            var status = new NamedPipeStatus();
            status.ICount = 255;
        ...
        private static NTCreateAndXResponse CreateResponseFromFileInformation(FileNetworkOpenInformation fileInfo, ushort fileID, FileStatus fileStatus)
        {
            ...
            response.ExtFileAttributes = fileInfo.FileAttributes;
        ...
        private static FileAttributes ToFileAttributes(ExtendedFileAttributes extendedFileAttributes)
        {
            ...
            var fileAttributes = FileAttributes.ReadOnly |
            ...
            return (fileAttributes & ((FileAttributes)extendedFileAttributes).Value);
        }
    }
}
