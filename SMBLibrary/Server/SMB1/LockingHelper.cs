...
namespace SMBLibrary.Server.SMB1
{
    internal class LockingHelper
    {
        internal static List<SMB1Command> GetLockingAndXResponse(SMB1Header header, LockingAndXRequest request, ISMBShare share, SMB1ConnectionState state)
        {
            ...
            for(var lockIndex = 0; lockIndex < request.Unlocks.Count; lockIndex++)
            {
                var lockingRange = request.Unlocks[lockIndex];
            ...
            {
                var lockingRange = request.Locks[lockIndex];
                var exclusiveLock = (request.TypeOfLock & LockType.SHARED_LOCK) == 0;
